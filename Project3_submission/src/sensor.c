...
int ConnectToNewServer(char server_reply[100])
{
	char **res,**res2,buff[255];
	struct sockaddr_in server;
	int count=0,sock;
	//printf("Hi i reached here server reply : %s\n",server_reply);
	split( server_reply, ':', &res, &count );

	split( res[2], ',', &res2, &count );
	...
	sock = socket(AF_INET , SOCK_STREAM , 0);
		...
		server.sin_addr.s_addr = inet_addr(res2[0]);
			server.sin_family = AF_INET;
			server.sin_port = htons( atoi(res2[1]) );
...
int main(int argc , char *argv[])
{
	...
	while(1){
		...
		{
			server_reply[n] = '\0';
			if(strstr(server_reply,"Connect")!=NULL)
			{
						printf("Received connect to replica : %s\n",server_reply);
						sock = ConnectToNewServer(server_reply);
						printf("New Socket ID : %d",sock);
						//n = sendregister(sock,sdevice);
						//pthread_create(&taskthread, NULL, SensorTemperatureHandler, (void*) &sock);
						goto label1;
			}
